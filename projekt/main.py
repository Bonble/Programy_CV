...
def scrape_url(url, q):
    ...
    #print("---")
    my_pickled_object = pickle.dumps([bank[1], produkt, proc_przed, proc_po, okres, min, max, anulowanie])  # Pickling the object
...
def main():
    ...
    for r in result_set:
        ...
        for x in n:
            ...
            val = (r[0], r[1][x], r[2][x], r[3][x], r[4][x], r[5][x].replace(' ',''), r[6][x].replace(' ',''), r[7][x])
...
def kursy():
    url = "http://api.nbp.pl/api/exchangerates/tables/C?formatJSON"
    ...
    lista = [0, 3, 5, 6]
    for x in lista:
        sql = "INSERT INTO kurs"+wynik[0]["rates"][x]["code"]+" (kupno, sprzedarz) VALUES (%s, %s)"
        val = (wynik[0]["rates"][x]["bid"], wynik[0]["rates"][x]["ask"])
        cursor.execute(sql, val)
        mydb.commit()
        
        sql = "DELETE FROM kurs" + wynik[0]["rates"][x]["code"] + " WHERE id = (SELECT MIN(id) FROM kurs" + wynik[0]["rates"][x]["code"] + ");"
        cursor.execute(sql)
        mydb.commit()

def krypto():
    url = "https://api.coingecko.com/api/v3/simple/price?ids=bitcoin%2Cethereum%2Ctether%2Cusd-coin%2Ceuro-coin%2Ctether-gold&vs_currencies=pln"

    headers = {
        "accept": "application/json",
        "x-cg-demo-api-key": "CG-ZMThumVeprtk1kemqLkB9NwF"
    }
    ...
    cursor = mydb.cursor()
    for x in wynik:
        sql = "INSERT INTO `kurs_" + x + "` (kurs) VALUES (%s)"
        val = wynik[x]["pln"]
        cursor.execute(sql, (val,))
        mydb.commit()

        sql = "DELETE FROM `kurs_" + x + "` WHERE id = (SELECT MIN(id) FROM `kurs_" + x + "`);"
        cursor.execute(sql)
        mydb.commit()
...
if __name__ == '__main__':
    main()
    print("lokaty zrobione")
    kursy()
    print("waluty zrobione")
    krypto()
    print("krypto zrobione")